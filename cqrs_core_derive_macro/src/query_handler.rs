...
pub fn implement_derive_query_handler_trait(derive_input: &DeriveInput) -> TokenStream {
    ...
    let expanded = quote! {
        impl cqrs_core::query_handler::QueryHandlerBase for #query_handler_struct_name
        {
            fn handle_query(&self, query_imp: &dyn cqrs_core::query::Query) -> Result<Box<dyn cqrs_core::query_response::QueryResponse>, Box<dyn std::error::Error>> {
