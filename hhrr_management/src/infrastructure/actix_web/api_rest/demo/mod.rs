...
pub fn demo_service_config(service_config: &mut web::ServiceConfig) -> () {
    service_config
        .service(web::resource("/query-user-by-id/{user_id}").route(web::get().to(query_user_by_id_controller::query_user_by_id_action)))
        .service(web::resource("/query-user-by-email/{user_email}").route(web::get().to(query_user_by_email_controller::query_user_by_email_action)))
